...
export const geocodeApi = {
  ...
  search: async (
    ...
    const response = await api.get(`/address/geocode/${encodeURIComponent(localCode)}`, {
      params: options,
    });
    return response.data;
  },
  ...
  /**
   * Get autocomplete suggestions for geocode search
   */
  autocomplete: async (
    localCode: string,
    limit: number = 10
  ): Promise<GeoCodeAutocompleteResponse> => {
    const response = await api.get(`/address/geocode/${encodeURIComponent(localCode)}/autocomplete`, {
      params: { limit },
    });
    return response.data;
  },
...
export const postalCodeApi = {
  ...
  ): Promise<PostalCodeDetailsResponse> => {
    const response = await api.get(`/zones/postal/${encodeURIComponent(postalCode)}/full`, {
      params: options,
    });
    return response.data;
  },
  ...
  autocomplete: async (
    query: string,
    limit: number = 10
  ): Promise<PostalCodeAutocompleteResponse> => {
    const response = await api.get('/zones/postal/autocomplete', {
      params: { q: query, limit },
    });
    return response.data;
  },
