...
class PlusCodeService:
    ...
    @classmethod
    def extract_local_code(cls, plus_code: str) -> Optional[str]:
        """
        Extract the 4-character local code from a Plus Code.

        The local code is the 4 characters immediately before the '+' separator.
        For example: '6WQPVX22+5WX' -> 'VX22'

        This is useful for local area identification and search.

        Args:
            plus_code: Full Plus Code string (e.g., "6WQPVX22+5WX")

        Returns:
            4-character local code (e.g., "VX22") or None if invalid
        """
        if not plus_code:
            return None
        ...
        # Check if it's a valid full code with a '+'
        ...
        # The local code is the last 4 characters before the '+'
        # Full codes have 8 chars before '+' (e.g., "6WQPVX22")
        # We want the last 4: "VX22"
        if len(before_plus) >= 4:
            return before_plus[-4:]

        return None

    @classmethod
    def is_valid_local_code(cls, local_code: str) -> bool:
        """
        Validate a 4-character local Plus Code.

        Args:
            local_code: 4-character string to validate

        Returns:
            True if valid (4 chars, all from Plus Code character set)
        """
        if not local_code or len(local_code) != 4:
            return False

        return all(c in cls.VALID_CHARS for c in local_code.upper())
