...
class GeocoderService:
    ...
    async def reverse_geocode(
        ...
        exact_match = None
        nearest_addresses = []
        first_address_zone_code = None

        for address, distance in rows:
            # Track the first (nearest) address's zone for fallback
            if first_address_zone_code is None:
                first_address_zone_code = address.zone_code
        ...
        # Fallback: If no containing zone found, use the nearest address's zone
        if zone is None and first_address_zone_code:
            fallback_stmt = select(PostalZone).where(PostalZone.zone_code == first_address_zone_code)
            fallback_result = await db.execute(fallback_stmt)
            zone = fallback_result.scalar_one_or_none()
