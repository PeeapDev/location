...
# =============================================================================
# Enhanced Postal Code Search Endpoints
# =============================================================================

class PostalAddressItem(BaseModel):
    """Address item in postal code details."""
    pda_id: str
    latitude: float
    longitude: float
    display_address: str
...
class PostalCodeDetailsResponse(BaseModel):
    """Full details response for postal code search."""
    zone_code: str
    primary_code: str
    segment: str
    ...
    plus_code_local: Optional[str]
    address_count: int
    addresses: List[PostalAddressItem]
    geometry: Optional[str]  # GeoJSON string
    query_time_ms: int


class PostalCodeAutocompleteItem(BaseModel):
    """Autocomplete suggestion for postal code search."""
    zone_code: str
    zone_name: Optional[str]
    district_name: str
    address_count: int
...
class PostalCodeAutocompleteResponse(BaseModel):
    """Response for postal code autocomplete."""
    suggestions: List[PostalCodeAutocompleteItem]
    query_time_ms: int


@router.get("/postal/{postal_code}/full", response_model=PostalCodeDetailsResponse)
async def get_postal_code_full(
    postal_code: str,
    include_addresses: bool = Query(True),
    address_limit: int = Query(50, ge=1, le=200),
    include_geometry: bool = Query(True),
    db: AsyncSession = Depends(get_db)
):
    """
    Get full details for a postal code including addresses and geometry.

    Supports both formats:
    - Primary code only: "1100" (returns first matching zone)
    - Full zone code: "1100-001"

    Returns zone details, all addresses within the zone, and the zone boundary.
    """
    ...
    if "-" in postal_code:
        # Full zone code
        stmt = select(PostalZone).where(PostalZone.zone_code == postal_code)
    else:
        # Primary code only - get first zone with this primary code
        stmt = select(PostalZone).where(PostalZone.primary_code == postal_code).limit(1)
    ...
    count_stmt = select(func.count(Address.pda_id)).where(Address.zone_code == zone.zone_code)
    count_result = await db.execute(count_stmt)
    address_count = count_result.scalar() or 0
    ...
    return PostalCodeDetailsResponse(
        zone_code=zone.zone_code,
        primary_code=zone.primary_code,
        segment=zone.segment,
        zone_name=zone.zone_name,
        ...
        plus_code=zone.plus_code,
        plus_code_local=zone.plus_code_local,
...
@router.get("/postal/autocomplete", response_model=PostalCodeAutocompleteResponse)
async def postal_code_autocomplete(
    q: str = Query(..., min_length=1, max_length=8),
    limit: int = Query(10, ge=1, le=20),
    db: AsyncSession = Depends(get_db)
):
    """
    Get autocomplete suggestions for postal code search.

    Supports partial input matching on:
    - Zone code (e.g., "11", "1100", "1100-")
    - Zone name
    - Primary code

    Returns zones with address counts for relevance.
    """
    start_time = time.time()

    q = q.strip()
    search_pattern = f"{q}%"
    search_pattern_contains = f"%{q}%"

    # Search zones matching the query
    stmt = (
        select(PostalZone)
        .where(
            or_(
                PostalZone.zone_code.like(search_pattern),
                PostalZone.primary_code.like(search_pattern),
                PostalZone.zone_name.ilike(search_pattern_contains)
            )
        )
    ...
    suggestions = []
    for zone in zones:
        # Get address count for each zone
        count_stmt = select(func.count(Address.pda_id)).where(Address.zone_code == zone.zone_code)
        count_result = await db.execute(count_stmt)
        addr_count = count_result.scalar() or 0

        suggestions.append(PostalCodeAutocompleteItem(
            zone_code=zone.zone_code,
            zone_name=zone.zone_name,
            district_name=zone.district_name,
            address_count=addr_count
    ...
    # Sort by address count (most addresses first)
    suggestions.sort(key=lambda x: x.address_count, reverse=True)
